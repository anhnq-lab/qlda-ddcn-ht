...
async function run() {
  const dummyProject = {
    project_id: 'TEST_BASE_DUMMY',
    project_name: 'Test Base Dummy Project'
  };

  console.log('Attempting base insert...');
  ...
  if (error) {
    ...
    const { error: delError } = await supabase.from('projects').delete().eq('project_id', 'TEST_BASE_DUMMY');
