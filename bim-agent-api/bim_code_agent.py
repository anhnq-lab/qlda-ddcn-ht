...
def run_python_code(code: str):
	...
	required_packages = ['pandas', 'numpy', 'matplotlib', 'plotly', 'seaborn', 'ifcopenshell', 'pyvista']
...
def init_multi_agent(tools_option, model_name="gpt-4o", init_db=False):
	...
	if os.path.exists("./vectorstore_db"):
		...
		for fname in input_files:
			...
			text_splitter = RecursiveCharacterTextSplitter(chunk_size=3000, chunk_overlap=500) # hyperparameters
...
def BIM_chain(inputs: Dict) -> Dict:
	...
	tools_prompt = f"""
	You are an expert in {selected_tools} field. Refer to the following ### Instruct and ### Example to generate executable Python code only without comments for ### User command.
