...
const ArticleContent: React.FC<{ content: string }> = ({ content }) => {
    ...
    return (
        <div
            className="text-[13px] leading-relaxed text-txt-secondary space-y-2 overflow-hidden break-words"
    ...
    return (
        <div
            ...
            className={`rounded-xl border transition-all duration-300 ${isTarget
                ...
                : 'bg-bg-surface border-border hover:border-gray-300 dark:hover:border-slate-600'
                }`}
        >
            <button
            ...
            >
                <div className="flex items-center gap-2 min-w-0">
                    ...
                    <span className={`text-sm font-semibold truncate ${isTarget ? 'text-primary-900 dark:text-primary-200' : 'text-txt-primary'}`}>
                        {article.title}
                    </span>
                </div>
                <div className={`shrink-0 p-1 rounded-lg transition-colors ${isExpanded ? 'bg-indigo-100 dark:bg-indigo-900/40' : 'bg-bg-muted'}`}>
                    {isExpanded
            ...
            {isExpanded && (
                <div className="px-4 pb-4 border-t border-border-subtle pt-3 overflow-hidden">
                    {article.summary && (
                        <p className="text-xs text-txt-muted italic mb-3 pb-3 border-b border-dashed border-gray-200 dark:border-slate-600 leading-relaxed">
                            {article.summary}
                        </p>
                    )}
                    {article.content && (
...
const LegalArticlePanel: React.FC<LegalArticlePanelProps> = ({ docId, articleId }) => {
    ...
    if (!doc) {
        return (
            <div className="flex flex-col items-center justify-center py-16 text-center px-8 h-full">
                ...
                <h3 className="text-sm font-bold text-txt-placeholder">Không tìm thấy văn bản</h3>
    ...
    return (
        <div className="flex flex-col h-full overflow-hidden">
            {/* Document Header */}
            <div className="px-5 py-4 border-b border-border bg-gradient-to-b from-gray-50/80 to-white dark:from-slate-800/80 dark:to-slate-900 shrink-0">
                {/* Badges */}
                ...
                {/* Title */}
                <h2 className="text-base font-bold text-txt-primary leading-snug mb-2">
                    {doc.title}
                </h2>

                {/* Code */}
                <p className="text-[10px] font-mono text-txt-placeholder mb-2 flex items-center gap-1.5">
                    <FileText className="w-3 h-3" />
                    {doc.code}
                </p>

                {/* Meta */}
                <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[10px] text-txt-muted">
                    <span className="flex items-center gap-1">
                        <Calendar className="w-3 h-3 text-gray-400" />
                        Ban hành: <strong className="text-txt-secondary">{doc.issued_date ? new Date(doc.issued_date).toLocaleDateString('vi-VN') : 'N/A'}</strong>
                    </span>
                    <span className="flex items-center gap-1">
                        <Shield className="w-3 h-3 text-emerald-500" />
                        Hiệu lực: <strong className="text-txt-secondary">{doc.effective_date ? new Date(doc.effective_date).toLocaleDateString('vi-VN') : 'N/A'}</strong>
                    </span>
                    <span className="flex items-center gap-1">
                        ...
                        >
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-xs font-semibold text-txt-primary group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                                        {chapter.title}
                                    </p>
                                </div>
            ...
            <div className="px-4 py-3 border-t border-border bg-gray-50/80 dark:bg-slate-800 shrink-0">
                <button
                    onClick={() => {
