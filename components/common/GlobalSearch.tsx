...
export const GlobalSearch: React.FC<GlobalSearchProps> = ({ isOpen, onClose, initialQuery = '' }) => {
    ...
    return (
        <div className="fixed inset-0 z-[200] flex items-start justify-center pt-[15vh] bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
            <div
                className="w-full max-w-2xl bg-white dark:bg-slate-800 rounded-2xl shadow-2xl overflow-hidden border border-gray-200 animate-in zoom-in-95 duration-200"
            ...
            >
                        ...
                        <div className="py-2">
                            {results.map((result, idx) => (
                                <button
                                    ...
                                    className={`w-full px-5 py-3 flex items-center gap-4 text-left transition-colors ${activeIndex === idx ? 'bg-blue-50' : 'hover:bg-slate-50 dark:bg-slate-800'
                                        }`}
                                >
                ...
                {/* Footer */}
                <div className="px-5 py-3 bg-slate-50 dark:bg-slate-800 border-t border-gray-200 flex items-center justify-between text-xs text-gray-400">
                    <div className="flex items-center gap-4">
                        <span className="flex items-center gap-1">
                            <kbd className="px-1.5 py-0.5 bg-white dark:bg-slate-800 border border-gray-200 rounded text-[10px] font-mono">↑</kbd>
                            <kbd className="px-1.5 py-0.5 bg-white dark:bg-slate-800 border border-gray-200 rounded text-[10px] font-mono">↓</kbd>
                            để di chuyển
                        </span>
                        <span className="flex items-center gap-1">
                            <kbd className="px-1.5 py-0.5 bg-white dark:bg-slate-800 border border-gray-200 rounded text-[10px] font-mono">Enter</kbd>
                            để chọn
                        </span>
                    </div>
