...
export const NotificationPage: React.FC = () => {
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in duration-300">
            {/* Header */}
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-bg-surface p-6 rounded-2xl border border-border shadow-card">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-xl font-extrabold text-txt-primary">Trung tâm thông báo</h1>
                ...
                {unreadCount > 0 && (
                    <button
                        ...
                        className="flex items-center gap-2 px-4 py-2.5 bg-bg-subtle hover:bg-bg-muted text-txt-secondary hover:text-txt-primary rounded-xl text-xs font-bold border border-border transition-all cursor-pointer shadow-card"
                    >
            ...
            {/* Filter buttons */}
            <div className="bg-bg-surface p-3 rounded-2xl border border-border shadow-card flex items-center gap-1.5 overflow-x-auto">
            ...
            {/* Notifications List */}
            <div className="bg-bg-surface rounded-2xl border border-border shadow-card overflow-hidden divide-y divide-border-subtle">
                {isLoading ? (
                    ...
                    <div className="py-24 text-center flex flex-col items-center justify-center">
                        <BellOff className="w-16 h-16 text-txt-placeholder mb-4" />
                        <h3 className="text-sm font-bold text-txt-secondary">Không tìm thấy thông báo</h3>
                    ...
                    <>
                        ...
                        {notifications.length >= limit && (
                            <div className="p-4 flex justify-center bg-bg-subtle">
                                <button
                                    onClick={handleLoadMore}
                                    className="px-6 py-2.5 bg-bg-surface hover:bg-bg-hover-row text-txt-secondary border border-border rounded-xl text-xs font-bold transition-all cursor-pointer shadow-card"
                                >
                                    Xem thêm thông báo
                                </button>
                            </div>
