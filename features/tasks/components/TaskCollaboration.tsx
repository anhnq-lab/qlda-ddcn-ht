...
export const TaskCollaboration: React.FC<TaskCollaborationProps> = ({ taskId, type }) => {
    ...
    if (type === 'comments') {
        return (
            <div className="flex flex-col h-full bg-bg-subtle relative">
                        ...
                        comments.map((comment: any) => (
                            <div key={comment.id} className="flex gap-4">
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="bg-bg-surface rounded-2xl rounded-tl-none border border-border p-4 shadow-sm">
                                        ...
                                        <div className="prose prose-sm dark:prose-invert max-w-none text-txt-muted" dangerouslySetInnerHTML={{ __html: comment.content }} />
                ...
                {/* Comment Input */}
                <div className="p-4 bg-bg-surface border-t border-border shrink-0">
                    <div className="border border-slate-200 dark:border-slate-600 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-blue-500/50 transition-all bg-bg-surface">
                        <ReactQuill
                            theme="snow"
                        ...
                        {attachments.length > 0 && (
                            <div className="px-3 py-2 bg-bg-subtle border-t border-border flex flex-wrap gap-2">
                                {attachments.map((file, idx) => (
                                    <div key={idx} className="flex items-center gap-2 bg-bg-surface border border-slate-200 dark:border-slate-600 rounded-md px-2 py-1 text-xs">
                                        <Paperclip className="w-3 h-3 text-slate-400" />
                                        <span className="truncate max-w-[150px] text-txt-muted">{file.name}</span>
                                        <button 
                        ...
                        <div className="flex items-center justify-between p-2 bg-bg-subtle border-t border-border">
                            <input 
                                type="file" 
                                className="hidden" 
                                ref={fileInputRef} 
    ...
    if (type === 'history') {
        return (
            <div className="p-4 md:p-6 h-full bg-bg-subtle">
                <h3 className="text-sm font-bold text-txt-primary mb-6 flex items-center gap-2">
                    ...
                    <div className="space-y-6 relative before:absolute before:inset-y-0 before:left-[19px] before:w-[2px] before:bg-slate-200 dark:before:bg-slate-700">
                        {activities.map((activity: any) => (
                            <div key={activity.id} className="relative flex items-start gap-4">
                                <div className="w-10 h-10 rounded-full bg-bg-surface border-2 border-border flex items-center justify-center shrink-0 z-10 text-xs font-bold text-slate-500">
