                ...
                <table className="w-full">
                    <thead className="sticky top-0 z-10 bg-slate-50 dark:bg-slate-800 text-[10px] font-black uppercase tracking-widest border-b border-slate-200 dark:border-slate-700 shadow-sm shadow-slate-200/20">
                        <tr className="text-slate-500 dark:text-slate-400">
                            <th className="px-2 py-2 w-10 border-b border-slate-200 dark:border-slate-700 text-center">
                                <input
                                    type="checkbox"
                            ...
                            <th className="px-2 py-2 text-center w-10 border-b border-slate-200 dark:border-slate-700">STT</th>
                            <th className="px-1.5 py-2 text-center w-8 border-b border-slate-200 dark:border-slate-700"></th>
                            <th onClick={() => handleSort('Title')} className="group/th px-2 py-2 text-left w-[25%] min-w-[160px] max-w-[200px] cursor-pointer select-none hover:text-blue-600 transition-colors border-b border-slate-200 dark:border-slate-700">
                                <span className="flex items-center gap-1">Công việc <SortIcon field="Title" /></span>
                            </th>
                            <th className="px-2 py-2 text-left hidden xl:table-cell w-[10%] min-w-[100px] border-b border-slate-200 dark:border-slate-700">Phân loại</th>
                            <th className="px-2 py-2 text-left hidden md:table-cell w-[10%] min-w-[90px] border-b border-slate-200 dark:border-slate-700">Phòng ban</th>
                            <th onClick={() => handleSort('ProgressPercent')} className="group/th px-2 py-2 text-center w-16 cursor-pointer select-none hover:text-blue-600 transition-colors border-b border-slate-200 dark:border-slate-700">
                                <span className="flex items-center justify-center gap-1">Tiến độ <SortIcon field="ProgressPercent" /></span>
                            </th>
                            <th className="px-2 py-2 text-left hidden lg:table-cell w-[12%] min-w-[110px] border-b border-slate-200 dark:border-slate-700">Phụ trách</th>
                            <th onClick={() => handleSort('Status')} className="group/th px-2 py-2 text-left hidden sm:table-cell w-24 cursor-pointer select-none hover:text-blue-600 transition-colors border-b border-slate-200 dark:border-slate-700">
                                <span className="flex items-center gap-1">Trạng thái <SortIcon field="Status" /></span>
                            </th>
                            <th onClick={() => handleSort('DueDate')} className="group/th px-2 py-2 text-left hidden sm:table-cell w-24 cursor-pointer select-none hover:text-blue-600 transition-colors border-b border-slate-200 dark:border-slate-700">
                                <span className="flex items-center gap-1">Hạn chót <SortIcon field="DueDate" /></span>
                            </th>
                            <th onClick={() => handleSort('Priority')} className="group/th px-2 py-2 text-center w-16 cursor-pointer select-none hover:text-blue-600 transition-colors border-b border-slate-200 dark:border-slate-700">
                                <span className="flex items-center justify-center gap-1">Ưu tiên <SortIcon field="Priority" /></span>
                            </th>
                            <th className="px-2 py-2 w-12 border-b border-slate-200 dark:border-slate-700"></th>
                        </tr>
                    </thead>
                                    ...
                                    <td colSpan={12} className="px-2.5 py-1.5">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <h3 className="text-[11px] font-bold text-slate-800 dark:text-slate-200 truncate">{getProjectName(projectId, projectTasks)}</h3>
                                                <p className="text-[9px] text-slate-400 dark:text-slate-400">{projectTasks.length} công việc</p>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
                            ...
                            {projectTasks.map((task, index) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {/* STT */}
                                        <td className="px-2 py-2 text-center text-[11px] text-slate-500 dark:text-slate-400 font-medium tabular-nums">
                                            {index + 1}
                                        </td>
                                        {/* Status */}
                                        ...
                                        <td className="px-2 py-2 pr-3 max-w-[200px]" title={task.Description || task.Title}>
                                            <div className="flex items-center gap-1.5 mb-0.5">
                                                <h4 className={`text-[12px] font-bold group-hover:text-blue-600 transition-colors line-clamp-1 ${task.Status === TaskStatus.Done ? 'text-slate-400 dark:text-slate-500' : isOverdue ? 'text-red-700 dark:text-red-400' : 'text-slate-800 dark:text-slate-100'
                                                    }`}>
                                            ...
                                            {task.Description && (
                                                <p className="text-[10px] text-slate-400 dark:text-slate-500 line-clamp-1 pr-4 truncate">{task.Description}</p>
                                            )}
                                        </td>
                                            ...
                                            {assignee ? (
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <div className="min-w-0">
                                                        <p className="text-[11px] font-semibold text-slate-700 dark:text-slate-300 truncate">{assignee.FullName}</p>
                                                        <p className="text-[9px] text-slate-400 dark:text-slate-500 truncate">{assignee.Position || assignee.Department}</p>
                                                    </div>
                                                </div>
                                            ) : (
                                        ...
                                        <td className="px-2 py-2 hidden sm:table-cell">
                                            {task.DueDate ? (
                                                <div className={`flex items-center gap-1 text-[11px] ${isOverdue ? 'text-red-600 font-semibold dark:text-red-400' : 'text-slate-500 dark:text-slate-400'}`}>
