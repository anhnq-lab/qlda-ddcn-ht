...
export const BIMAgentChat: React.FC<BIMAgentChatProps> = ({ onClose, isDark = false, embedded = false }) => {
    ...
    return (
        <div className={`flex flex-col h-full transition-all duration-300 ${isExpanded && embedded ? 'fixed inset-4 z-50 rounded-xl shadow-2xl' : 'w-full h-full'}`}
            ...
            {/* Header */}
            <div className={`flex items-center justify-between px-4 py-3 border-b ${isDark ? 'border-slate-800 bg-slate-900/50' : 'border-gray-200 bg-gray-50'}`}>
                ...
                {embedded && (
                    <div className="flex items-center gap-1">
                        <button onClick={() => setIsExpanded(!isExpanded)} className={`p-1.5 rounded-md hover:bg-slate-500/20 text-slate-400 transition-colors`}>
                            ...
                            {msg.results && msg.results.length > 0 && (
                                <div className="mt-3 space-y-3">
                                    {msg.results.map((r, j) => {
                                        ...
                                        return (
                                            <div key={j} className={`rounded-lg overflow-hidden ${isDark ? 'bg-slate-900/50' : 'bg-white'} border ${isDark ? 'border-slate-700' : 'border-gray-200'}`}>
