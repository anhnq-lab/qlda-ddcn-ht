...
const OverviewTab: React.FC<{ contractor: ContractorData, contractorId: string, projectId?: string }> = ({ contractor, contractorId, projectId }) => {
    ...
    return (
        <div className="space-y-6">
            ...
            {/* General Information */}
            <div className="bg-bg-surface rounded-xl border border-border overflow-hidden shadow-sm shadow-gray-200/50 dark:shadow-none">
                <div className="px-4 py-3 bg-bg-subtle dark:bg-slate-750 border-b border-border">
                ...
                return (
                    <div key={ct.contract_id} className="bg-bg-surface rounded-xl border border-border p-4 hover:shadow-md transition-shadow">
                        ...
                        <div className="mt-3 flex flex-wrap items-center gap-3 text-xs">
                            ...
                            {ct.contract_type && (
                                <span className="text-txt-muted bg-bg-muted px-2 py-0.5 rounded-md">
                                    {ct.contract_type}
                                </span>
                            )}
                            {ct.sign_date && (
            ...
            {packages.map(pkg => (
                ...
                >
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-3 text-xs">
                        ...
                        {pkg.field && (
                            <span className="text-txt-muted bg-bg-muted px-2 py-0.5 rounded-md uppercase text-[10px] font-bold">
                                {pkg.field}
                            </span>
                        )}
                        {pkg.selection_method && (
                            <span className="text-txt-muted pt-0.5">
                                {pkg.selection_method}
                            </span>
                        )}
                    </div>
                        ...
                        <div className="flex items-center gap-3 min-w-0">
                            ...
                            <div className="min-w-0">
                                ...
                                <div className="flex items-center gap-2 mt-1">
                                    <span className="text-[10px] text-txt-muted bg-bg-muted px-1.5 py-0.5 rounded font-medium">
                                        {doc.project_name}
                                    </span>
                                    {doc.doc_type && (
...
const LoadingList = () => (
    <div className="space-y-3">
        {[1, 2, 3].map(i => (
            <div key={i} className="h-24 bg-bg-subtle rounded-xl animate-pulse" />
        ))}
    </div>
);
