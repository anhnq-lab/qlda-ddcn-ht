...
const STATUS_CONFIG: Record<AssetStatus, { label: string; color: string; darkColor: string; icon: React.ReactNode }> = {
    ...
    Retired: { label: 'Ngừng SD', color: 'bg-gray-100 text-gray-500', darkColor: 'bg-slate-50 text-slate-400', icon: <XCircle className="w-3 h-3" /> },
};
...
export const FacilityManagementPanel: React.FC = () => {
    ...
    const inputCls = `w-full px-3 py-2 text-xs rounded-lg border outline-none transition-colors ${isDarkMode
        ? 'bg-slate-50 border-slate-600 text-white placeholder:text-slate-500 focus:border-blue-500'
    ...
    return (
        <div className="w-full">
            ...
            <div className={`flex items-center gap-2 px-3 pb-1.5 ${isMobile ? 'flex-wrap' : ''}`}>
                <div className={`flex items-center gap-2 flex-1 min-w-[160px] px-2.5 py-1.5 rounded-lg ${isDarkMode ? 'bg-slate-50' : 'bg-gray-100'}`}>
                ...
                <select
                    ...
                    className={`text-xs px-2 py-1.5 rounded-lg border ${isDarkMode ? 'bg-slate-50 border-slate-600 text-slate-300' : 'bg-white dark:bg-slate-800 border-gray-200 text-gray-600'}`}
                >
                ...
                <div className="overflow-x-auto overflow-y-auto flex-1 custom-scrollbar">
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <td className="px-3 py-1.5">
                                            {asset.category ? (
                                                <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-medium ${isDarkMode ? 'bg-slate-50 text-slate-300' : 'bg-gray-100 text-gray-600'
                                                    }`}>
                    ...
                    {filteredAssets.length === 0 && !loading && (
                        <div className="flex flex-col items-center justify-center py-8 text-center">
                            <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-3 ${isDarkMode ? 'bg-slate-50' : 'bg-gray-100'}`}>
