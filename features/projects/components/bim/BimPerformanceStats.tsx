    ...
    return (
        <div
            className={`absolute bottom-14 left-3 z-40 rounded-xl backdrop-blur-xl border shadow-sm overflow-hidden select-none
                ${isDarkMode ? 'bg-slate-50 border-slate-700/50 text-slate-300' : 'bg-white/90 border-gray-200 text-gray-700'}
            `}
            style={{ minWidth: 180, fontSize: '11px' }}
        >
            {/* Header */}
            <div className={`flex items-center justify-between px-3 py-1.5 border-b
                ${isDarkMode ? 'border-slate-700/50 bg-slate-50' : 'border-gray-200 bg-slate-50 dark:bg-slate-800'}
            `}>
            ...
            <div className="px-3 py-2 space-y-1">
                ...
                <div className={`h-px my-1 ${isDarkMode ? 'bg-slate-50' : 'bg-gray-100'}`} />
