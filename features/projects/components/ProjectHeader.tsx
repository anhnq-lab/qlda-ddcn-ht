...
export const ProjectHeader: React.FC<ProjectHeaderProps> = ({ project, onSync, isSyncing, syncResult, onDelete, onEdit, compact, hideBackButton }) => {
    ...
    if (compact) {
        return (
            <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-800 rounded-xl shadow-sm overflow-hidden">
                ...
                {!hideBackButton && (
                    ...
                    <span className="hidden md:inline-flex text-[11px] font-bold text-gray-600 dark:text-slate-300 bg-slate-50 dark:bg-slate-800 px-1.5 py-0.5 rounded border border-gray-200 dark:border-slate-700 shrink-0">
    ...
    return (
        <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden">
            {/* Top section - Header */}
                ...
                <div className="flex flex-wrap items-start justify-between gap-4">
                    <div className="flex items-start gap-4 flex-1 min-w-0">
                        {!hideBackButton && (
                            <button
                                onClick={() => navigate(-1)}
                                className="p-2 hover:bg-white dark:bg-slate-800 dark:hover:bg-slate-700 rounded-xl transition-all border border-transparent hover:border-gray-200 dark:hover:border-slate-600 bg-white dark:bg-slate-800 shadow-sm"
                            >
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <div className="flex flex-wrap items-center gap-4 text-sm font-medium">
                                ...
                                {/* Budget */}
                                <span className="text-xs font-bold text-gray-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-800 px-2 py-0.5 rounded-md border border-gray-200 dark:border-slate-700">
                                    {formatShortCurrency(project.TotalInvestment)}
                                </span>
                            </div>
