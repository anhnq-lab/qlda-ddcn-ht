...
export const AIReportModal: React.FC<AIReportModalProps> = ({
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center">
            ...
            <div className="relative w-full max-w-4xl max-h-[92vh] bg-bg-surface rounded-2xl shadow-sm border border-border flex flex-col overflow-hidden">

                {/* ── Header ── */}
                <div className="flex items-center justify-between px-5 py-3 border-b border-border bg-gradient-to-r from-primary-50 to-warning-50 dark:from-primary-900/20 dark:to-warning-900/10">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-sm font-black text-txt-primary">Báo Cáo Tháng — AI</h2>
                            <p className="text-[10px] text-txt-muted truncate max-w-xs">{projectName}</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-1.5">
                        ...
                        <button onClick={onClose} className="p-2 rounded-lg hover:bg-gray-200 dark:hover:bg-slate-600 transition-colors text-txt-placeholder ml-1">
                            <X className="w-4 h-4" />
                        </button>
                    </div>
                </div>
...
const ActionBtn: React.FC<{
    ...
    <button
        ...
        className={`flex items-center gap-1 px-2.5 py-1.5 text-[10px] font-bold rounded-lg transition-all ${
            ...
            'bg-bg-surface hover:bg-bg-subtle dark:hover:bg-slate-600 text-txt-muted border border-gray-200 dark:border-slate-600'
        }`}
    >
...
const LoadingState = () => (
    <div className="flex flex-col items-center justify-center py-20 gap-5">
        ...
        <div className="text-center">
            <p className="text-sm font-bold text-txt-secondary">AI đang tổng hợp báo cáo...</p>
            <p className="text-xs text-txt-placeholder mt-1">Phân tích dữ liệu dự án và soạn báo cáo tháng</p>
        </div>
        <div className="w-full max-w-md space-y-3 mt-2 px-8">
            {[90, 75, 85, 60, 70, 50].map((w, i) => (
                <div key={i} className="h-3.5 bg-bg-muted rounded-md animate-pulse" style={{ width: `${w}%`, animationDelay: `${i * 150}ms` }} />
            ))}
        </div>
    </div>
);
...
const ErrorState: React.FC<{ error: string; onRetry: () => void }> = ({ error, onRetry }) => (
    <div className="flex flex-col items-center justify-center py-20 gap-4">
        ...
        <p className="text-[11px] text-txt-placeholder max-w-sm text-center leading-relaxed">{error}</p>
