...
export const EventSlidePanel: React.FC<EventSlidePanelProps> = ({ event, onEdit, onClose }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50/50 dark:bg-slate-900/50">
      {/* Header Info Banner */}
      <div className="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 px-6 py-5 shrink-0">
        <div className="flex items-start justify-between gap-4">
          ...
          {canEditOrDelete && (
            <div className="flex gap-2 shrink-0">
              ...
              <Button variant="outline" size="sm" className="text-red-600 hover:text-red-700 hover:bg-red-50 border-red-200" onClick={handleDelete} loading={deleteMutation.isPending}>
      ...
      {/* Tabs */}
      <div className="flex items-center gap-6 px-6 border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shrink-0">
        <button
      ...
      <div className="flex-1 overflow-y-auto">
        {activeTab === 'info' && (
          <div className="p-6 space-y-6">
            <div className="bg-white dark:bg-slate-800 rounded-xl p-5 border border-slate-200 dark:border-slate-700 shadow-sm space-y-5">
              {(event.room || event.location) && (
                <div className="flex items-start text-sm text-slate-700 dark:text-slate-300">
                  <div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-700 flex items-center justify-center mr-3 shrink-0">
              ...
              {event.description && (
                <div className="flex items-start text-sm text-slate-700 dark:text-slate-300">
                  ...
                  <div className="pt-1.5">
                    <p className="font-medium mb-1">Nội dung / Chương trình</p>
              ...
              <div className="flex items-start text-sm text-slate-700 dark:text-slate-300">
                ...
                <div className="pt-1.5 w-full">
                  <p className="font-medium mb-2">Thành phần tham dự ({event.attendees?.length || 0})</p>
                  {event.attendees && event.attendees.length > 0 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-2">
                      {event.attendees.map((emp, idx) => (
                        <div key={emp.EmployeeID || (emp as any).id || idx} className="flex items-center gap-3 p-2 rounded-lg border border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50">
                    ...
                    <p className="text-slate-500 italic mt-1">Chưa có thông tin</p>
                  )}
                </div>
              </div>
            </div>
          ...
          <div className="p-6 space-y-6">
            <div className="bg-white dark:bg-slate-800 rounded-xl p-5 border border-slate-200 dark:border-slate-700 shadow-sm space-y-4">
              <div>
                ...
                <textarea
                  className="w-full min-h-[150px] p-3 text-sm rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
