...
const CDEWorkflowPanel: React.FC<CDEWorkflowPanelProps> = ({
    ...
    return (
        <div className="w-[340px] bg-bg-surface rounded-2xl shadow-sm border border-border flex flex-col overflow-hidden shrink-0 animate-in slide-in-from-right-4 duration-300">
            {/* Header */}
            <div className="px-4 py-3.5 border-b border-border flex justify-between items-center bg-bg-subtle">
                <span className="text-[10px] font-black text-txt-muted uppercase tracking-[0.15em]">Phê duyệt hồ sơ</span>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-5">
                ...
                <div className="flex items-start gap-3 pb-4 border-b border-border">
                    ...
                    <div className="min-w-0">
                        <h4 className="text-sm font-bold text-txt-primary line-clamp-2">{doc.doc_name}</h4>
                ...
                {/* Workflow Progress */}
                <div>
                    <h5 className="text-[10px] font-black text-txt-muted uppercase tracking-wider mb-3">Tiến trình duyệt</h5>
                            ...
                            return (
                                <div key={idx} className="flex flex-col items-center gap-1.5 flex-1 relative">
                                    {idx < CDE_WORKFLOW_STEPS.length - 1 && (
                                        <div className={`absolute left-1/2 right-[-50%] top-3 h-0.5 z-0 ${isCompleted ? 'bg-emerald-500' : 'bg-bg-muted'}`} />
                                    )}
                ...
                <div className={`rounded-xl p-3.5 border ${colors.lightBg} ${colors.border}`}>
                    <p className="text-[10px] font-bold text-txt-placeholder uppercase mb-1.5">Trạng thái hiện tại</p>
                ...
                {workflowHistory.length > 0 && (
                    <div>
                        <h5 className="text-[10px] font-black text-txt-muted uppercase tracking-wider mb-3 flex items-center gap-1.5">
                        ...
                        <div className="relative pl-4 space-y-3 before:absolute before:left-[5px] before:top-2 before:bottom-2 before:w-0.5 before:bg-gray-100 dark:before:bg-slate-700">
                            {[...workflowHistory].reverse().map((hist, idx) => (
                                <div key={idx} className="relative">
                                    ...
                                    <div>
                                        <p className="text-xs font-bold text-txt-primary">{hist.step_name}</p>
            ...
            {/* Action Buttons */}
            <div className="p-4 border-t border-border bg-gray-50/80 dark:bg-slate-800">
                {nextStep ? (
                    <div className="space-y-2.5">
                        ...
                        <p className="text-[9px] text-txt-placeholder text-center font-medium">
