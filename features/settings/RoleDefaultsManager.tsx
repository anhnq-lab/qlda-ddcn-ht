...
const RoleDefaultsManager: React.FC = () => {
    ...
    return (
        <div className="space-y-6">
                    ...
                    return (
                        <button
                            ...
                            className={`flex items-center gap-2 px-3 py-2.5 rounded-xl border text-left transition-all ${isActive
                                ...
                                : 'border-border hover:border-primary-300 dark:hover:border-primary-600 bg-bg-surface'
                                }`}
                        >
                            <Shield size={16} className={isActive ? 'text-primary-600 dark:text-primary-400' : 'text-slate-400'} />
                            <div className="min-w-0 flex-1">
                                <p className="text-[13px] font-bold text-txt-primary truncate">{ROLE_LABELS[role]}{subLabel}</p>
                                <p className="text-[11px] text-txt-muted">{permCount[role]} quyền</p>
                            </div>
            ...
            <div className="bg-bg-surface rounded-2xl border border-border-subtle shadow-sm overflow-hidden flex-1 flex flex-col min-h-0">
                {/* ─── Selected Role Header ─── */}
                <div className="flex items-center justify-between px-6 py-4 border-b border-border-subtle bg-slate-50/50 dark:bg-slate-800">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button
                            ...
                            className="flex items-center gap-1.5 px-4 py-1.5 text-sm font-semibold text-txt-secondary bg-bg-muted hover:bg-slate-200 dark:hover:bg-slate-700 rounded-lg transition-colors shadow-sm disabled:opacity-50"
                        >
                            <RefreshCw size={14} />
                            Đồng bộ cho tất cả
                        </button>
                    ...
                    <table className="w-full text-sm">
                        <thead className="sticky top-0 z-10 bg-bg-subtle text-[10px] font-black uppercase tracking-widest border-b border-border shadow-sm shadow-slate-200/20">
                            <tr className="text-txt-muted">
                                <th className="text-left px-6 py-3 w-56 border-b border-border">
                                    Phân hệ
                                </th>
                                ...
                                <th className="text-center px-2 py-3 w-16 border-b border-border">
                                    Tất cả
                                </th>
                            </tr>
                        </thead>
                        <tbody>
                            {PHASE1_RESOURCES.map((resource, idx) => {
                                ...
                                return (
                                    <tr
                                        key={resource}
                                        className={`border-b border-border-subtle hover:bg-slate-50/80 dark:hover:bg-slate-700 transition-colors ${idx % 2 === 0 ? 'bg-white dark:bg-transparent' : 'bg-slate-50/30 dark:bg-slate-800'
                                            }`}
                                    >
                                                ...
                                                <td key={action} className="text-center px-2 py-2.5">
                                                    <button
                                                        ...
                                                        className={`w-7 h-7 mx-auto rounded-md border-2 flex items-center justify-center transition-all ${hasAction
                                                            ...
                                                            : 'border-slate-200 dark:border-slate-600 bg-bg-subtle hover:border-primary-300 dark:hover:border-primary-500 text-transparent hover:text-primary-300'
                                                            }`}
                                                    >
            ...
            {showApplyConfirm && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                    <div className="bg-bg-surface rounded-2xl p-6 max-w-md w-full mx-4 shadow-2xl border border-border animate-in fade-in zoom-in-95 duration-200">
                        <div className="flex items-start gap-4 mb-5">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-txt-muted leading-relaxed">
                                    ...
                                    <span className="font-semibold text-txt-primary">"{ROLE_LABELS[selectedRole]}"</span>.
                                </p>
                            </div>
                        </div>
