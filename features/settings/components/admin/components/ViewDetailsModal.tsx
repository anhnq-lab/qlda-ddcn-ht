...
export const ViewDetailsModal: React.FC<ViewDetailsModalProps> = ({ viewingAccount, onClose, onOpenResetModal }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={onClose}>
            <div className="bg-bg-surface rounded-2xl w-full max-w-lg shadow-xl overflow-hidden" onClick={e => e.stopPropagation()}>
                <div className="p-6 bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-900 border-b border-border">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-txt-muted font-medium mt-1">
                ...
                <div className="p-6 space-y-6">
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1">
                            <span className="text-xs font-semibold text-txt-muted uppercase">Username</span>
                            <p className="font-mono text-sm font-medium text-txt-primary bg-bg-muted px-2 py-1 rounded inline-block">
                                {viewingAccount.username}
                            </p>
                        </div>
                        <div className="space-y-1">
                            <span className="text-xs font-semibold text-txt-muted uppercase">Phân quyền</span>
                    ...
                    <div className="space-y-3">
                        <h3 className="text-sm font-bold text-txt-primary flex items-center gap-2 pb-2 border-b border-border-subtle">
                            <ShieldCheck className="w-4 h-4 text-primary-500" />
                            Bảo mật & Đăng nhập
                        </h3>
                        
                        <div className="grid grid-cols-[120px_1fr] gap-2 text-sm">
                            <div className="text-txt-muted">Email:</div>
                            <div className="font-medium text-txt-primary">{viewingAccount.email || '—'}</div>
                            
                            <div className="text-txt-muted">SĐT:</div>
                            <div className="font-medium text-txt-primary">{viewingAccount.phone || '—'}</div>
                            
                            <div className="text-txt-muted">Đăng nhập cuối:</div>
                            <div className="font-medium text-txt-primary">{formatDate(viewingAccount.last_login)}</div>
                            
                            <div className="text-txt-muted">Trạng thái:</div>
                            <div>
                            ...
                            <div className="text-txt-muted mt-2">Mật khẩu:</div>
                ...
                <div className="p-4 border-t border-border-subtle bg-bg-subtle flex justify-end">
                    <button
                        onClick={onClose}
                        className="px-5 py-2 bg-bg-surface border border-slate-200 dark:border-slate-600 rounded-xl text-sm font-medium hover:bg-slate-50 dark:hover:bg-slate-600 transition-colors"
                    >
                        Đóng
                    </button>
                </div>
