    ...
    return (
        <div className="flex flex-col gap-2 mb-3">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-lg font-black text-gray-800 dark:text-slate-100 tracking-tight uppercase flex items-center gap-2">
                        <Scale className="w-5 h-5 text-primary-600 dark:text-primary-400" />
                        Văn bản Pháp luật Xây dựng
                    </h2>
                    <p className="text-[11px] text-gray-500 dark:text-slate-400 mt-0.5 font-medium ml-7 hidden sm:block">
                        Tra cứu thông minh Luật, Nghị định, Thông tư, Quy chuẩn về xây dựng và đầu tư công
                    </p>
                </div>
                <div className="flex items-center gap-2 text-[11px] font-bold">
                    <div className="flex items-center gap-1.5 px-2.5 py-1.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-600 dark:text-slate-300">
                        <FileText className="w-3.5 h-3.5 text-slate-400" />
                        Tổng: <span className="text-slate-900 dark:text-slate-100">{stats.total}</span>
                    </div>
            ...
            <div className="flex items-center gap-3">
                <div className="relative flex-1">
                    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 flex items-center px-3 h-10 transition-all focus-within:ring-2 focus-within:ring-primary-100 dark:focus-within:ring-primary-900/40 focus-within:border-primary-400">
                        <Search className="w-4 h-4 text-gray-400 dark:text-slate-400 mr-2" />
                        <input
                            type="text"
                            placeholder="Tìm theo số hiệu, tên văn bản, nội dung điều khoản..."
                            className="flex-1 h-full outline-none text-[13px] font-medium text-gray-700 dark:text-slate-300 placeholder-gray-400 dark:placeholder-slate-500 bg-transparent"
                            value={searchQuery}
                    ...
                    {showDeepSearch && deepSearchResults.length > 0 && (
                        <div className="absolute top-14 left-0 right-0 z-50 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-700 max-h-96 overflow-y-auto p-3 space-y-2 animate-in slide-in-from-top-2 duration-200">
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                    <Filter className="w-3.5 h-3.5 text-gray-400 dark:text-slate-400 mr-1" />
