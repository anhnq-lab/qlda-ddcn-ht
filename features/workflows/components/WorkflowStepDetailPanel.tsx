...
export const WorkflowStepDetailPanel: React.FC<WorkflowStepDetailPanelProps> = ({
  ...
  return (
    <div className="flex flex-col h-full bg-bg-surface border-l border-border">
      <div className="p-4 border-b border-border bg-bg-surface sticky top-0 z-10">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-bg-muted">{task?.status || 'Active'}</span>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-6 animate-fade-in">
            <Section title="Mô tả công việc & SOP" icon={FileText}>
              {meta.guidelines || meta.description ? (
                ...
                <p className="text-sm text-txt-secondary">Chưa có mô tả.</p>
              )}
            </Section>
              ...
              <Section title="Chi tiết các công việc thành phần" icon={CheckSquare} color="emerald">
                <ul className="space-y-3">
                  {meta.sub_tasks.map((st: any) => (
                    <li key={st.id} className="flex items-start gap-3 p-3 bg-bg-subtle rounded-xl border border-border-subtle">
                      <div className="mt-0.5"><CheckCircle size={16} className="text-slate-300 dark:text-slate-600" /></div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-txt-secondary leading-snug">{st.name}</p>
...
const Section: React.FC<{ title: string; icon: React.ElementType; children: React.ReactNode; color?: string }> = ({ 
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="bg-bg-surface p-4 rounded-xl border border-border-subtle shadow-sm">{children}</div>
    </div>
  );
};
