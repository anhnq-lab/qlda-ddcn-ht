      ...
      if (step.subtasks && step.subtasks.length > 0) {
        ...
        for (let idx = 0; idx < subCount; idx++) {
          ...
          const { error: tErr } = await supabase.from('tasks').insert({
            ...
            estimated_cost: subCost,
            actual_cost: status === 'done' ? subCost : Math.round(subCost * (progress / 100)),
        ...
        const { error: tErr } = await supabase.from('tasks').insert({
          ...
          actual_cost: status === 'done' ? step.estimatedCost : Math.round(step.estimatedCost * (progress / 100)),
          legal_basis: step.legalBasis,
          output_document: step.outputDocument,
          sort_order: i * 10,
          predecessor_task_id: previousTaskId,
